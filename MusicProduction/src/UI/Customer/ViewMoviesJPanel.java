...
public class ViewMoviesJPanel extends javax.swing.JPanel {
    ...
    UserAccount account;
    Studio studio;
    public ViewMoviesJPanel(JPanel workArea, UserAccount account, MusicStudioSystem system) {
        initComponents();
    ...
    private void populateCombo() {
        for(Network network:system.getNetworkList()) {
            for (Studio stu: network.getEnterpriseDirectory().getEnterpriseList()) {
                if (stu.getEnterpriseType().toString().equals("CinemaEn")) {
                    cmbCinema.addItem(stu);
                }
            }
        }
    }
    
    private void populateTable(Studio stu) {
        DefaultTableModel model = (DefaultTableModel) infoTable.getModel();
        model.setRowCount(0);
        for(Organization org: stu.getOrganizationDirectory().getOrganizationList()) {
    ...
    private void btnStartOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnStartOrderActionPerformed
        ...
        StartOrderJPanel jp = new StartOrderJPanel(workArea, account, studio, movie, system);
    ...
    private void cmbCinemaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmbCinemaActionPerformed
        studio = (Studio)cmbCinema.getSelectedItem();
        populateTable(studio);
...
//        lblMovieType.setText(crq.getMovie().getType());
    ...
    private void populateByMName(String name) {
        ...
        for(Network network:system.getNetworkList()) {
            for (Studio stu: network.getEnterpriseDirectory().getEnterpriseList()) {
                if (studio.getEnterpriseType().toString().equals("CinemaEn")) {
                    for(Organization org: stu.getOrganizationDirectory().getOrganizationList()) {
                        for(UserAccount ua: org.getUserAccountDirectory().getUserAccountList()) {
                            if (ua.getRole().toString().equals("CinemaManager")) {
                                for(CinemaRequest crq : ua.getCinemaQueue().getCinemaRequestList()) {
